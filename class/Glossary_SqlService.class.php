...
//	var $mConfig = array();
	...
	function getFeaturedWords($sqlPram = '',$configUseModRewrite = false, $configDesrLength = 200) 
	{
	...
	// Categiry Word
	// --------------------------------------------------------
	function getCategoryWords($categoryId=0, $configPerPage=10, $startEntry=0, $configDesrLength= 200, $configUseModRewrite = false ) 
	{
		$root =& XCube_Root::getSingleton();
		$db =& $root->mController->mDB;
		$textFilter =& $root->getTextFilter();
	...
	// --------------------------------------------------------
	function getCategoryItemCount($categoryId=0) 
	{
	...
	// --------------------------------------------------------
	// SQL Function get tag items
	// --------------------------------------------------------
	function getTagItems($getTag='', $startEntry=0, $configPerPage=20, $configDesrLength=100, $configUseModRewrite=false) 
	{
		$root =& XCube_Root::getSingleton();
		$db =& $root->mController->mDB;
		$textFilter =& $root->getTextFilter();
		$tagHandler =& xoops_getmodulehandler('tag');
		$retArray = array();
		$sql = 'SELECT w.wordid, w.categoryid ,w.term, w.proc, w.init, w.description, c.name FROM '. $db->prefix("glossary_word")
			.' w LEFT JOIN '. $db->prefix("glossary_tag").' t ON w.wordid = t.wordid LEFT JOIN '. $db->prefix("glossary_category").' c ON w.categoryid = c.categoryid '.
			' WHERE w.published > 0 ';
		$sql .= " AND t.tag = '$getTag' ORDER BY w.term ASC";
		if ($db->query($sql)) {
			$result = $db->query($sql, $configPerPage, $startEntry );
			while (list($wordId, $categoryId, $term, $proc, $init, $wordDesc, $categoryName) = $db->fetchRow($result)) {
				$wordUrl = Glossary_functionGetWordUrl( $configUseModRewrite, intval($wordId), addSlashes($term), addSlashes($categoryName) );
				// delete for HTML tag and javaa script
				$wordDesc = $textFilter->toShowTarea($wordDesc, $html=1, $smiley=1, $xcode=0, $image=1, $br=1,false);
				$wordDesc = preg_replace('!<script.*?>.*?</script.*?>!is', '', $wordDesc);
				$wordDesc = strip_tags($wordDesc);
				if ( !XOOPS_USE_MULTIBYTES ) {
	...
	function countTagItems($getTag='') 
	{
		...
		if ($getTag) {
			...
			$sql .= " AND t.tag = '$getTag' ORDER BY w.term ASC";
			$result = $db->query($sql);
			$totalCount = $db -> getRowsNum($result);
			if ($totalCount > 0) {
				return $totalCount;
			} else {
				return false;
	...
	// --------------------------------------------------------
	// SQL Function get letter item
	// --------------------------------------------------------
	function getLetterItems($dbName='', $selFieldName='', $orderFiled='', $orderValue='') {
		$root =& XCube_Root::getSingleton();
		$db =& $root->mController->mDB;
		$textFilter =& $root->getTextFilter();
		$tagHandler =& xoops_getmodulehandler('tag');
	...
	function getRss($categoryid = 0 ,$configUseModRewrite = false) 
	{
		$root =& XCube_Root::getSingleton();
		$textFilter =& $root->getTextFilter();
		$db =& $root->mController->mDB;
		...
		$retArray = array();
		$sql = "SELECT c.categoryid, c.name, w.wordid, term, w.english, proc, w.description ,w.published FROM ". $db -> prefix("glossary_category")." c ".
			"LEFT JOIN ". $db -> prefix("glossary_word")." w ON c.categoryid = w.categoryid WHERE published > 0 ORDER BY submited DESC  LIMIT 10";
		$result = $db -> query($sql);
		while (list($categoryId, $categoryName, $wordId, $term, $english, $proc, $wordDesc, $wordPublished) = $db -> fetchRow($result)) {
			$wordDesc = $textFilter->toShowTarea($wordDesc, $html=1, $smiley=1, $xcode=0, $image=1, $br=1,false);
			$wordDesc = preg_replace('!<script.*?>.*?</script.*?>!is', '', $wordDesc);
			...
			$blolckWord = array(
				...
				'date'   => intval($wordPublished),
				'link' => Glossary_functionGetWordUrl( $configUseModRewrite, intval($wordId), addSlashes($term), addSlashes($categoryName) ),
			);
			array_push($retArray , $blolckWord );
			unset ($blolckWord) ;
		}
