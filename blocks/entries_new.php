...
function glossary_b_entries_new_show($options ) {
	...
	$sql = "SELECT c.categoryid, c.name, w.wordid, w.term FROM ". $xoopsDB -> prefix("glossary_category")." c ".
		"LEFT JOIN ". $xoopsDB -> prefix("glossary_word")." w ON c.categoryid = w.categoryid WHERE published > 0 ORDER BY submited DESC";
	...
	if ( $totalwords > 0 ) {
		while (list($categoryId, $categoryName, $wordId, $wordTerm) = $xoopsDB -> fetchRow($result)) {
			...
			$newentries['linkWordUrl'] = Glossary_functionGetWordUrl( $configUseModRewrite, intval($wordId), addSlashes($wordTerm));
...
function glossary_b_entries_new_edit($options ) {
